...
public class SplitTerrain : EditorWindow {
    ...
    Transform transform;
    int xLen = 5; // Match with TerrainManager if using
    int zLen = 5;
    ...
    void copyTerrain(Terrain origTerrain, string newName, float xMin, float xMax, float zMin, float zMax, int heightmapResolution, int detailResolution, int alphamapResolution) {
        ...
        AssetDatabase.CreateAsset(td, "Assets/" + newName + ".asset");
        ...
        newTerrain.preserveTreePrototypeLayers = origTerrain.preserveTreePrototypeLayers;
        ...
        newTerrain.treeLODBiasMultiplier = origTerrain.treeLODBiasMultiplier;
        ...
        newTerrain.allowAutoConnect = true;// I don't think this is needed rn
        ...
        for (int i = 0; i < heightmapResolution; i++) {
            for (int j = 0; j < heightmapResolution; j++) {
                Vector3 origPos = origTerrain.transform.position;   // to make it world space
                // Divide by size.y because height is stored as percentage
                // Note this is [j, i] and not [i, j] (Why?!)
                newHeights[j, i] = origTerrain.SampleHeight(new Vector3(origPos.x + xMin + (i * dimRatio1), 0, origPos.z + zMin + (j * dimRatio2))) / origTerrain.terrainData.size.y;
            }
        }
